...
def hash_carpeta(ruta_carpeta, extensiones=None, excluir_archivos=None):
    ...
    for archivo in sorted(ruta.rglob("*")):
        ...
        # Evita que el propio JSON cambie el hash en cada ejecución
    ...
    return hash_total.hexdigest(), archivos_incluidos
...
def main():
    ...
    output_file = source_dir / "version_manifest.json"

    hash_actual, archivos_incluidos = hash_carpeta(
        ruta_source,
        extensiones=extensiones,
        excluir_archivos={"version_manifest.json"}
    )
    ...
    contenido = {
        ...
        "algoritmo": "SHA256",
        "extensiones_consideradas": sorted(list(extensiones)),
        "cantidad_archivos_incluidos": len(archivos_incluidos),
